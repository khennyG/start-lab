...
export default function PromptEngineeringClient() {
	...
	const flipCards = [
		...
		},
		{
			...
			const buildSteps = [
			{
				icon: <TargetIcon className="h-12 w-12 text-red-600" />,
			...
			{
				icon: <FileTextIcon className="h-12 w-12 text-green-600" />,
	...
	const nextExample = () => {
		...
		return (
			<div className="min-h-screen bg-white">
				{/* What Is Prompt Engineering - Flip Cards */}
					...
					{flipCards.map((card) => (
						...
						>
							...
							>
								{/* Front */}
								<div
									className={`absolute w-full h-full backface-hidden ${card.color} rounded-2xl shadow-lg p-8 flex items-center justify-center border-2 border-gray-200`}
								>
								...
								{/* Back */}
								<div
									className={`absolute w-full h-full backface-hidden ${card.color} rounded-2xl shadow-lg p-8 flex items-center justify-center rotate-y-180 border-2 border-gray-200`}
								>
			...
			<div id="build-better" className="max-w-6xl mx-auto px-6 py-20">
				<h2 className="text-4xl font-bold text-gray-900 mb-4 text-center">
				...
				<div className="grid md:grid-cols-3 gap-6 mb-8">
					{buildSteps.map((step, idx) => (
						...
						>
							<div className="mb-4 group-hover:animate-bounce">{step.icon}</div>
							<div
								className={`inline-block ${step.color} text-white px-4 py-2 rounded-full text-lg font-bold mb-3`}
							>
								Step {idx + 1}
							</div>
